<template>
  <div class="myspace-wrap">
    <div class="options-box">
      <div class="option-title flex-box">
        <div>
          短链分组<span> 共{{ editableTabs?.length }}组</span>
        </div>
        <div class="hover-box add-group-btn" @click="showAddGroup">
        ...
        <li v-for="(item, index) in editableTabs" :key="item.gid">
          <div class="item-box flex-box hover-box" :class="{ selectedItem: selectedIndex === index }"
            ...
            <div class="group-name-wrap">
              <img src="@/assets/svg/移动竖.svg" width="13" alt="" />
              <el-tooltip show-after="500" :content="item.name">
                <span class="over-text">{{ item.name }}</span>
              </el-tooltip>
            </div>
            <div class="item-actions flex-box">
              <!-- 图标 -->
      ...
      <div class="table-box">
        ...
        <div v-if="!isRecycleBin" class="buttons-box">
          <div class="buttons-row">
            ...
            <el-button class="batchButton" @click="isAddSmallLinks = true">批量创建</el-button>
          </div>
        </div>
        <!-- 展示回收站信息 -->
  ...
  Sortable.create(table, {
    ...
    onEnd: async ({ oldIndex, newIndex }) => {
      ...
      if (newIndex !== oldIndex) {
        ...
        const data = transformGroupData(oldIndex, newIndex)
        await API.group.sortGroup(data)
      }
    }
  })
}
...
const queryRecycleBinPage = async () => {
  const res = await API.smallLinkPage.queryRecycleBin({ current: pageParams.current, size: pageParams.size })
  tableData.value = res.data?.data?.records
  totalNums.value = +res.data?.data?.total
...
.hover-box:hover {
  cursor: pointer;
  color: #385c89;
  background: linear-gradient(145deg, #eff4fb 0%, #e8eef7 100%);
  box-shadow:
    6px 6px 12px rgba(156, 173, 198, 0.28),
...
.options-box {
  ...
  background: linear-gradient(145deg, #edf2f9 0%, #e7edf6 100%);
  box-shadow:
    10px 10px 20px rgba(158, 175, 199, 0.3),
...
.selectedItem {
  ...
  background: linear-gradient(145deg, #eaf0f8 0%, #e4ebf5 100%) !important;
  box-shadow:
    inset 4px 4px 8px rgba(163, 178, 201, 0.3),
...
.content-box {
  ...
  .table-box {
    ...
    .batchButton {
      ...
      border: 0;
      box-shadow:
        5px 5px 10px rgba(161, 177, 201, 0.3),
        -5px -5px 10px rgba(255, 255, 255, 0.9);
      transition: all 0.18s cubic-bezier(0.4, 0, 0.2, 1);
    }

    .addButton {
      background: linear-gradient(145deg, #537fc3 0%, #426da9 100%);
    }
    ...
    .batchButton:active {
      ...
      box-shadow:
        inset 4px 4px 8px rgba(131, 149, 177, 0.34),
        inset -4px -4px 8px rgba(255, 255, 255, 0.82);
    }

    .batchButton {
      color: #3f5f8f;
      background: linear-gradient(145deg, #eef3fa 0%, #e6edf7 100%);
    }
...
:deep(.el-dialog) {
  border-radius: 18px;
  background: linear-gradient(145deg, #eef3fa 0%, #e8eef7 100%);
  box-shadow:
    7px 7px 14px rgba(148, 166, 192, 0.34),
